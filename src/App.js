...
function App() {
  ...
  const playSoundByKey = useCallback(
    (key) => {
      ...
      <div className="drum-machine">
        {currentBank.samples.map((sample, index) => (
          <DrumPad
            key={index}
            keyLabel={sample.key}
            samplePath={currentBank.basePath + sample.file}
          />
        ))}
        <button className="control-pad play-pause" onClick={handlePlayPause}>
          ▶
        </button>
        <button className="control-pad restart" onClick={handleRestartTrack}>
          ←
        </button>
      </div>
    </div>
